...
Future<List<Commande>> getAllCommande() async {
  ...
  try {
    ...
    if (request.statusCode == 200 || request.statusCode == 201) {
      ...
      throw Exception('Erreur lors de la récupération des commandes : ${request.statusCode}');
    }
  } catch (error) {
...
Future<Article> getArticleDetails(String articleId) async {
  ...
  final articleRequest = await http.get(Uri.parse(artUrl));
  ...
  if (articleRequest.statusCode == 200 || articleRequest.statusCode == 201) {
    ...
    throw Exception('Erreur lors de la récupération des détails de l\'article : ${articleRequest.statusCode}');
  }
}
}